...
class NestedCrossValidation:
    ...
    #I will set this class to work on two modes - optimize=False no inner loop (training on the outer loop) and optimize=True (inner loop for Optuna for hyperparameters tuning and evaluation on the outer loop)
    ...
    def runcv(self):
        ...
        #Use RepeatedStratifiesKFold to ensure proper class imbalance handling
        ...
        for name, model in self.estimators:
            ...
            for train_index, test_index in outer_cv.split(self.X, self.y):
                ...
                if should_optimize and (name in self.param_space):
                    ...
                    best_params = self.optuna(X_train, y_train, clone(model), self.param_space[name])
